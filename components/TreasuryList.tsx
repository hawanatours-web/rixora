...
const TreasuryList: React.FC = () => {
  ...
  const handlePrintReport = () => {
    ...
    const accountName = selectedTreasuryFilter === 'ALL' ? 'جميع الصناديق والبنوك' : treasury.find(t => t.id === selectedTreasuryFilter)?.name || 'غير معروف';
    ...
    const reportHTML = `
      ...
      <head>
        <title>تقرير حركة الخزينة</title>
        <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;600;700;800&display=swap" rel="stylesheet">
        <style>
            @page { size: A4 ${isCheckReport ? 'landscape' : ''}; margin: 15mm; }
            body { font-family: 'Cairo', sans-serif; background: white; color: #1e293b; padding: 10px; direction: rtl; zoom: 0.85; }
      ...
      <body>
        <div class="container">
            <div class="header">
                ...
                <div class="meta">
                    <div>التاريخ: <span style="font-family: monospace;">${currentDate}</span></div>
                    <div>الحساب: ${accountName}</div>
                </div>
            </div>

            <div class="title">${isCheckReport ? 'تقرير الشيكات (حافظة الشيكات)' : 'تقرير حركة الخزينة الكامل'}</div>
            ...
            <table>
                ...
                <tbody>
                    ${reportTransactions.length > 0 ? reportTransactions.map(t => `
                    <tr>
                        ...
                        ${isCheckReport ? `
                            ...
                            <td><span>${t.checkDetails?.status || 'غير محدد'}</span></td>
                        ` : `
                            <td>${t.type}</td>
                    ...
                    `).join('') : '<tr><td colspan="7" style="text-align:center; padding:20px; color: #94a3b8;">لا توجد حركات</td></tr>'}
                </tbody>
            </table>

            <div class="footer">${companySettings.nameAr} - جميع الحقوق محفوظة</div>
        </div>
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-8">
        ...
        <div className="fixed inset-0 bg-slate-900/50 dark:bg-slate-950/90 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-fade-in">
            <div className="bg-white dark:bg-[#1e293b] w-full max-w-md rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden">
                ...
                <form onSubmit={handleSubmitTransfer} className="p-6 space-y-4">
                    <div className="bg-cyan-50 dark:bg-cyan-900/20 p-3 rounded-lg border border-cyan-200 dark:border-cyan-800 text-sm">
                        <p className="text-slate-500 dark:text-slate-400 mb-1">تفاصيل الحركة:</p>
                        <p className="font-bold text-slate-800 dark:text-white">{transactionToTransfer.description}</p>
                        <p className="font-mono font-bold text-cyan-600 dark:text-cyan-400 mt-1">{displayMoney(transactionToTransfer.amount)} JOD</p>
                    </div>

                    <div>
                        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">نقل من الحساب الحالي:</label>
                        <div className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded p-2 text-slate-500 dark:text-slate-400 text-sm">
                            {treasury.find(t => t.id === transactionToTransfer.treasuryId)?.name || 'غير معروف'}
                        </div>
                    </div>

                    <div>
                        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">إلى الحساب (الهدف):</label>
                        <select 
                            required 
                        ...
                        >
                            ...
                            {treasury.filter(t => t.id !== transactionToTransfer.treasuryId).map(t => (
                                <option key={t.id} value={t.id}>{t.name} (الرصيد: {displayMoney(t.balance)})</option>
                            ))}
                        </select>
                    </div>

                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={() => setIsTransferModalOpen(false)} className="flex-1 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300 rounded-lg transition-colors">إلغاء</button>
                        <button type="submit" className="flex-1 py-2 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-lg transition-colors">تأكيد النقل</button>
                    </div>
                </form>
            </div>
        </div>
      ...
      {isCheckModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 dark:bg-slate-950/90 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-fade-in">
            <div className="bg-white dark:bg-[#1e293b] w-full max-w-md rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden">
                <div className="bg-slate-50 dark:bg-[#0f172a] p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
                    <h3 className="text-slate-800 dark:text-white font-bold flex items-center gap-2">
                        <ScrollText size={20} className="text-purple-500 dark:text-purple-400" />
                        تسجيل شيك وارد
                    </h3>
                ...
                <form onSubmit={handleSubmitCheck} className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                             ...
                             <input required type="text" value={checkData.checkNumber} onChange={(e) => setCheckData({...checkData, checkNumber: e.target.value})} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-slate-800 dark:text-white focus:border-purple-500 focus:outline-none font-mono" />
                        </div>
                        <div>
                             <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">تاريخ الاستحقاق *</label>
                             <input required type="date" value={checkData.dueDate} onChange={(e) => setCheckData({...checkData, dueDate: e.target.value})} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-slate-800 dark:text-white focus:border-purple-500 focus:outline-none" />
                        </div>
                    </div>
                    
                    <div>
                        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">البنك المسحوب عليه</label>
                        <input type="text" value={checkData.bankName} onChange={(e) => setCheckData({...checkData, bankName: e.target.value})} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-slate-800 dark:text-white focus:border-purple-500 focus:outline-none" placeholder="مثال: البنك العربي" />
                    </div>

                    <div>
                        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">اسم الساحب (العميل)</label>
                        <input type="text" value={checkData.clientName} onChange={(e) => setCheckData({...checkData, clientName: e.target.value})} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-slate-800 dark:text-white focus:border-purple-500 focus:outline-none" />
                    </div>

                    <div className="bg-slate-100 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-200 dark:border-slate-700 space-y-3">
                        <div>
                             <label className="block text-xs text-purple-600 dark:text-purple-400 font-bold mb-1">قيمة الشيك (Original Amount)</label>
                             <div className="flex gap-2">
                                <input 
                                    required
                                    type="number" 
                                    ...
                                    className="flex-1 bg-slate-50 dark:bg-[#0f172a] border border-purple-500/50 dark:border-purple-700 rounded-lg p-2 text-slate-800 dark:text-white font-bold focus:outline-none" 
                                />
                                <select 
                                    value={checkCurrency}
                                    onChange={(e) => setCheckCurrency(e.target.value as Currency)}
                                    className="w-24 bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded-lg p-2 text-slate-800 dark:text-white text-sm focus:border-purple-500 focus:outline-none font-bold"
                                >
                        ...
                        <div className="flex gap-3">
                            <div className="flex-1">
                                <label className="block text-[10px] text-slate-500 dark:text-slate-500 mb-1">سعر الصرف</label>
                                <input 
                                    type="number" 
                                    step="any"
                                    value={checkExchangeRate}
                                    onChange={(e) => setCheckExchangeRate(e.target.value)}
                                    className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-slate-800 dark:text-white text-sm focus:border-purple-500 focus:outline-none"
                                />
                            </div>
                            <div className="flex-1">
                                <label className="block text-[10px] text-slate-500 dark:text-slate-500 mb-1">القيمة (JOD)</label>
                    ...
                    <div>
                        <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">ملاحظات / بيان</label>
                        <input type="text" value={checkData.description} onChange={(e) => setCheckData({...checkData, description: e.target.value})} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-slate-800 dark:text-white focus:border-purple-500 focus:outline-none" />
                    </div>

                    <button type="submit" className="w-full py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-lg mt-2 transition-colors">
                        حفظ الشيك
                    </button>
                </form>
            </div>
      ...
      {/* Delete Treasury Confirmation Modal */}
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 dark:bg-slate-950/90 backdrop-blur-sm z-[60] flex justify-center items-center p-4 animate-fade-in">
            <div className="bg-white dark:bg-[#1e293b] w-full max-w-sm rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xl p-6 text-center">
                ...
                <h3 className="text-slate-800 dark:text-white text-lg font-bold mb-2">حذف الحساب</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">هل أنت متأكد من حذف هذا الحساب؟ تأكد من عدم وجود حركات مرتبطة به.</p>
                <div className="flex gap-3">
        ...
        <div className="fixed inset-0 bg-slate-900/50 dark:bg-slate-950/90 backdrop-blur-sm z-[60] flex justify-center items-center p-4 animate-fade-in">
            <div className="bg-white dark:bg-[#1e293b] w-full max-w-sm rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xl p-6 text-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
                    هل أنت متأكد من التراجع عن هذه الحركة؟<br/>
                    سيتم حذف الحركة وعكس تأثيرها المالي على رصيد الصندوق.
                </p>
                <div className="flex gap-3">
                    <button onClick={() => setIsUndoModalOpen(false)} className="flex-1 py-2.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-white rounded-lg transition-colors">إلغاء</button>
