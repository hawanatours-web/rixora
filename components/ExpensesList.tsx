...
const ExpensesList: React.FC = () => {
  ...
  const handlePrintVoucher = (expense: Transaction) => {
    ...
    const voucherHTML = `
      ...
      <head>
        <title>سند صرف رقم ${expense.referenceNo || expense.id}</title>
        <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;600;700;800&display=swap" rel="stylesheet">
        <style>
            @page { size: A5 landscape; margin: 10mm; }
            body { font-family: 'Cairo', sans-serif; margin: 0; padding: 0; color: #1e293b; background: white; }
            .container {
            ...
            .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #334155; padding-bottom: 10px; margin-bottom: 15px; }
            .company-info { text-align: right; }
            .company-name { font-size: 16px; font-weight: 800; color: #0f172a; }
            .company-contact { font-size: 10px; color: #64748b; margin-top: 2px; }
            
            .voucher-title { text-align: center; }
            .voucher-title h1 { margin: 0; font-size: 22px; font-weight: 900; color: #334155; border: 2px solid #334155; padding: 5px 20px; border-radius: 10px; background-color: #f8fafc; }
            .voucher-title p { margin: 5px 0 0; font-size: 12px; font-weight: bold; letter-spacing: 1px; }

            .meta-info { text-align: left; font-size: 12px; font-weight: bold; }
            .meta-row { margin-bottom: 5px; }

            .content { flex-grow: 1; display: flex; flex-direction: column; gap: 12px; padding-top: 5px; }
            
            .field-row { display: flex; align-items: baseline; font-size: 14px; font-weight: 600; }
            .field-label { width: 110px; flex-shrink: 0; color: #334155; font-weight: 700; }
            .field-value { flex-grow: 1; border-bottom: 1px dotted #334155; padding-bottom: 2px; padding-right: 10px; color: #0f172a; font-weight: 700; min-height: 20px; }
            ...
            .amount-label { font-size: 9px; font-weight: bold; border-bottom: 1px solid #ccc; padding-bottom: 2px; margin-bottom: 2px; }
            .amount-number { font-size: 18px; font-weight: 900; direction: ltr; }

            .footer { display: flex; justify-content: space-between; margin-top: 10px; padding-top: 10px; }
            .sig-box { text-align: center; width: 30%; }
            .sig-title { font-size: 11px; font-weight: bold; margin-bottom: 35px; }
            .sig-line { border-bottom: 1px dashed #334155; width: 80%; margin: 0 auto; }
      ...
      <body>
        <div class="container">
            <div class="header">
                ...
                <div class="meta-info">
                    <div class="meta-row">رقم السند: <span style="color: #ef4444; font-family: monospace; font-size: 14px;">${expense.referenceNo || expense.id}</span></div>
                    <div class="meta-row">التاريخ: <span style="font-family: monospace;">${new Date(expense.date).toLocaleDateString('en-GB')}</span></div>
                </div>
            </div>

            <div class="content">
                <div class="field-row">
                    <span class="field-label">يصرف إلى السيد/ة:</span>
                    <span class="field-value">${expense.description}</span>
                </div>
                
                <div class="field-row">
                    <span class="field-label">مبلغ وقدره:</span>
                    <span class="field-value">${convertAmount(expense.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })} ${systemCurrency} (فقط لا غير)</span>
                </div>
                ...
                <div class="field-row">
                    ...
                    <span class="field-value">${expense.checkDetails ? 'شيك' : 'نقداً / تحويل'} (من: ${treasury.find(t => t.id === expense.treasuryId)?.name || 'الصندوق'})</span>
                </div>
            </div>

            <div class="amount-section">
                <div class="amount-box">
                    <div class="amount-label">المبلغ (${systemCurrency})</div>
