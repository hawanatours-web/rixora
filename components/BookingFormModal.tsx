...
import { Plus, X, Trash2, FileText, User, Box, Briefcase, Clock, Sparkles, Loader2, Info, Phone } from 'lucide-react';
import { useData } from '../context/DataContext';
import { getFlightDetails } from '../services/geminiService';
...
const BookingFormModal: React.FC<BookingFormModalProps> = ({ editingId, onClose }) => {
    ...
    }, [editingId, bookings, treasury, convertAmount]);
    ...
    const handleAddRoute = (serviceId: string) => {
        setServices(services.map(s => {
            if (s.id === serviceId) {
                ...
                return { ...s, routes: [...currentRoutes, { id: `r-${Date.now()}`, from: '', to: '' }] };
            }
            return s;
        }));
    };
    ...
    // AI Flight Search
    const handleFetchFlightInfo = async (serviceId: string, flightNo: string, date: string) => {
        if (!flightNo || !date) {
            showNotification('يرجى إدخال رقم الرحلة وتاريخ الذهاب', 'error');
            return;
        }
        setIsFlightLoading(serviceId);
                            ...
                            {services.map((s, i) => (
                                <div key={s.id} className="bg-white dark:bg-[#1e293b] rounded-xl border border-slate-200 dark:border-slate-700 p-4 relative">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 pl-8">
                                        <select value={s.type} onChange={(e) => handleServiceChange(s.id, 'type', e.target.value)} className="bg-slate-50 dark:bg-[#020617] border border-slate-300 dark:border-slate-600 rounded p-2" disabled={!!s.inventoryId}><option value="Flight">طيران</option><option value="Hotel">فندق</option><option value="Visa">تأشيرة</option><option value="Transport">نقل</option><option value="Tour">رحلة سياحية</option><option value="Insurance">تأمين سفر</option><option value="Other">أخرى</option></select>
                                        <select value={s.supplier || ''} onChange={(e) => handleServiceChange(s.id, 'supplier', e.target.value)} className="bg-slate-50 dark:bg-[#020617] border border-slate-300 dark:border-slate-600 rounded p-2" disabled={!!s.inventoryId}><option value="">اختر مورد...</option>{agents.map(a => <option key={a.id} value={a.name}>{a.name}</option>)}</select>
                                    </div>
                                    
                                    {s.type === 'Flight' && (
                                        <div className="mb-4 space-y-3">
                                            <div className="flex items-end gap-2 mb-2">
                                                <div className="flex-1"><label className="block text-[10px] text-slate-500 mb-1">تاريخ الذهاب (للبحث)</label><input type="date" value={s.flightDate || ''} onChange={(e) => handleServiceChange(s.id, 'flightDate', e.target.value)} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-xs" /></div>
                                                <div className="flex-1"><label className="block text-[10px] text-slate-500 mb-1">رقم الرحلة (للبحث)</label><input type="text" value={s.ticketNumber || ''} onChange={(e) => handleServiceChange(s.id, 'ticketNumber', e.target.value)} placeholder="Ex: RJ 261" className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-xs" /></div>
                                                <button type="button" onClick={() => handleFetchFlightInfo(s.id, s.ticketNumber || '', s.flightDate || '')} disabled={isFlightLoading === s.id} className="bg-cyan-600 text-white p-2 rounded hover:bg-cyan-500 disabled:opacity-50 flex items-center gap-1 text-xs h-[34px]">{isFlightLoading === s.id ? <Loader2 size={14} className="animate-spin"/> : <Sparkles size={14}/>} جلب البيانات</button>
                                            </div>
                                            <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                                                <div><label className="block text-[10px] text-slate-500 mb-1">خط الطيران</label><input type="text" value={s.airline || ''} onChange={(e) => handleServiceChange(s.id, 'airline', e.target.value)} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-xs" /></div>
                                                ...
                                                <div><label className="block text-[10px] text-slate-500 mb-1">PNR</label><input type="text" value={s.pnr || ''} onChange={(e) => handleServiceChange(s.id, 'pnr', e.target.value)} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-xs" /></div>
                                            </div>
                                            <div className="bg-slate-50 dark:bg-[#0f172a] p-3 rounded border border-slate-200 dark:border-slate-700">
                                                ...
                                                <div className="space-y-2">
                                                    {(s.routes && s.routes.length > 0) ? null : (
                                                        <div className="flex items-center gap-2"><input type="text" value={s.route || ''} onChange={(e) => handleServiceChange(s.id, 'route', e.target.value)} placeholder="Main Route (e.g. AMM-DXB)" className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-xs" /></div>
                                                    )}
                                                    {(s.routes || []).map((route) => (
                                                        <div key={route.id} className="flex items-center gap-2">
                                                            <div className="flex-1"><input type="text" value={route.from} onChange={(e) => handleUpdateRoute(s.id, route.id, 'from', e.target.value)} placeholder="من (From)..." className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-xs" /></div>
                                                            <span className="text-slate-400">➝</span>
                                                            <div className="flex-1"><input type="text" value={route.to} onChange={(e) => handleUpdateRoute(s.id, route.id, 'to', e.target.value)} placeholder="إلى (To)..." className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-xs" /></div>
                                                            <button onClick={() => handleRemoveRoute(s.id, route.id)} className="text-red-400 hover:text-red-600"><X size={14} /></button>
                                                        </div>
                                                    ))}
                                                    <button onClick={() => handleAddRoute(s.id)} className="text-xs flex items-center gap-1 text-cyan-600 dark:text-cyan-400 font-bold mt-2 hover:underline"><Plus size={12} /> إضافة مقطع (Segment)</button>
                                                </div>
                                            </div>
                                        </div>
                                    )}

                                    {/* ... (Other Service Types handling - keeping logic from previous) ... */}
                                    {s.type === 'Hotel' && (
                                    ...
                                    {s.type === 'Transport' && (
                                        <div className="mb-4 space-y-3">
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                <div><label className="block text-[10px] text-slate-500 mb-1">نوع المركبة</label><input type="text" value={s.vehicleType || ''} onChange={(e) => handleServiceChange(s.id, 'vehicleType', e.target.value)} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-xs" placeholder="Bus / Car" /></div>
                                                <div><label className="block text-[10px] text-slate-500 mb-1">اسم السائق</label><input type="text" value={s.driverName || ''} onChange={(e) => handleServiceChange(s.id, 'driverName', e.target.value)} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-xs" /></div>
                                                <div><label className="block text-[10px] text-slate-500 mb-1">تاريخ التنقل</label><input type="date" value={s.transportDate || ''} onChange={(e) => handleServiceChange(s.id, 'transportDate', e.target.value)} className="w-full bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600 rounded p-2 text-xs" /></div>
                                            </div>
                                            <div className="bg-slate-50 dark:bg-[#0f172a] p-3 rounded border border-slate-200 dark:border-slate-700">
                                                <h5 className="text-xs font-bold text-slate-600 dark:text-slate-300 mb-2">خط السير (Segments)</h5>
                                                <div className="space-y-2">
                                                    ...
                                                    <button onClick={() => handleAddRoute(s.id)} className="text-xs flex items-center gap-1 text-cyan-600 dark:text-cyan-400 font-bold mt-2 hover:underline"><Plus size={12} /> إضافة خط سير (Segment)</button>
                                                </div>
                                            </div>
                                        </div>
                                    )}
