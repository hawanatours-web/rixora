...
const getClient = () => {
    ...
    if (!key || key.length < 10) {
        console.warn("Gemini Service: API Key is missing or invalid. AI features will be disabled.");
        return null;
    }
...
export const analyzeFinancialQuery = async (query: string, contextData: string): Promise<string> => {
  try {
    const ai = getClient();
    if (!ai) return "عذراً، خدمة الذكاء الاصطناعي غير مفعلة حالياً. يرجى التحقق من إعدادات النظام (API Key missing).";
